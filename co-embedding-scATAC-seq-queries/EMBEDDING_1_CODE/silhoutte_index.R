...
data_mint <- read.csv('tsne_silhoutte/case_1_mint.csv', header = FALSE,sep=",")
drops <- c("V1","V3","V5")
data_mint = data_mint[ , !(names(data_mint) %in% drops)]
colnames(data_mint) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_mint, grepl('Human-Ex Neuron|Mouse-Neuron', X))
data2 = dplyr::filter(data_mint, grepl('Human-HSC|Mouse-HSC', X))
data3 = dplyr::filter(data_mint, grepl('Mouse-Bcell|Human-GM|Human-GM-D', X))
data4 = dplyr::filter(data_mint, grepl('Human-Myoblast', X))
data1$X = "neuron"
data2$X = "hsc"
data3$X = "bcell"
data4$X = "myoblast"
data_mint = rbind(data1,data2,data3,data4)
colnames(data_mint) = c("TSNE_1","TSNE_2","X")
d1 = as.data.frame(apply(data_mint,2,function(x) {x<-as.numeric(factor(x,levels = unique(x)))}))
data_mint$X = d1$X
dim(data_mint)

data_scanorama <- read.csv('tsne_silhoutte/scanorama_case1.csv',sep=",",header=FALSE)
data_scanorama <- data_scanorama[-c(1)]
colnames(data_scanorama) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_scanorama, grepl('Human-Ex Neuron|Mouse-Neuron', X))
data2 = dplyr::filter(data_scanorama, grepl('Human-HSC|Mouse-HSC', X))
data3 = dplyr::filter(data_scanorama, grepl('Mouse-Bcell|Human-GM|Human-GM-D', X))
data4 = dplyr::filter(data_scanorama, grepl('Human-Myoblast', X))
data1$X = "neuron"
data2$X = "hsc"
data3$X = "bcell"
data4$X = "myoblast"
data_scanorama = rbind(data1,data2,data3,data4)
colnames(data_scanorama) = c("TSNE_1","TSNE_2","X")
d1 = as.data.frame(apply(data_scanorama,2,function(x) {x<-as.numeric(factor(x,levels = unique(x)))}))
data_scanorama$X = d1$X
dim(data_scanorama)

data_scale <- read.csv('tsne_silhoutte/scale_case1.csv',sep=",",header=FALSE)
data_scale <- data_scale[-c(1)]
colnames(data_scale) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_scale, grepl('Human-Ex Neuron|Mouse-Neuron', X))
data2 = dplyr::filter(data_scale, grepl('Human-HSC|Mouse-HSC', X))
data3 = dplyr::filter(data_scale, grepl('Mouse-Bcell|Human-GM|Human-GM-D', X))
data4 = dplyr::filter(data_scale, grepl('Human-Myoblast', X))
data1$X = "neuron"
data2$X = "hsc"
data3$X = "bcell"
data4$X = "myoblast"
data_scale = rbind(data1,data2,data3,data4)
colnames(data_scale) = c("TSNE_1","TSNE_2","X")
d1 = as.data.frame(apply(data_scale,2,function(x) {x<-as.numeric(factor(x,levels = unique(x)))}))
data_scale$X = d1$X
dim(data_scale)

data_scvi <- read.csv('tsne_silhoutte/scvi_case1.csv',sep=",",header=FALSE)
data_scvi <- data_scvi[-c(1)]
colnames(data_scvi) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_scvi, grepl('Human-Ex Neuron|Mouse-Neuron', X))
data2 = dplyr::filter(data_scvi, grepl('Human-HSC|Mouse-HSC', X))
data3 = dplyr::filter(data_scvi, grepl('Mouse-Bcell|Human-GM|Human-GM-D', X))
data4 = dplyr::filter(data_scvi, grepl('Human-Myoblast', X))
data1$X = "neuron"
data2$X = "hsc"
data3$X = "bcell"
data4$X = "myoblast"
data_scvi = rbind(data1,data2,data3,data4)
colnames(data_scvi) = c("TSNE_1","TSNE_2","X")
d1 = as.data.frame(apply(data_scvi,2,function(x) {x<-as.numeric(factor(x,levels = unique(x)))}))
data_scvi$X = d1$X
dim(data_scvi)

data_scepi <- read.csv('tsne_silhoutte/scepi_pos_case1.csv',sep=",",header=FALSE)
data_scepi <- data_scepi[-c(1)]
colnames(data_scepi) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_scepi, grepl('Human-Ex Neuron|Mouse-Neuron', X))
data2 = dplyr::filter(data_scepi, grepl('Human-HSC|Mouse-HSC', X))
data3 = dplyr::filter(data_scepi, grepl('Mouse-Bcell|Human-GM|Human-GM-D', X))
data4 = dplyr::filter(data_scepi, grepl('Human-Myoblast', X))
data1$X = "neuron"
data2$X = "hsc"
data3$X = "bcell"
data4$X = "myoblast"
