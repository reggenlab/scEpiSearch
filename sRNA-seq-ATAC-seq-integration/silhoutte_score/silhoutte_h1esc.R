...
data_seurat <- read.csv('embeds-MCA-cell-Human-HESC.txt', header = TRUE,sep="\t")
head(data_seurat,3)
data_seurat = data_seurat[ , c(-1)]
colnames(data_seurat) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_seurat, grepl('Embryonic-Stem-Cell|H1ESC', X))
data2 = dplyr::filter(data_seurat, !grepl('Embryonic-Stem-Cell|H1ESC', X))
...
data_conos <- read.csv('H1ESC_conos_tsne.txt',header=TRUE,sep=",")
head(data_conos,3)
data_conos <- data_conos[-c(1)]
colnames(data_conos) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_conos, grepl('Embryonic-Stem-Cell|H1ESC', X))
data2 = dplyr::filter(data_conos, !grepl('Embryonic-Stem-Cell|H1ESC', X))
...
data_liger <- read.csv('liger_tsne_h1esc.txt', header = TRUE,sep="\t")
head(data_liger,3)
data_liger <- data_liger[-c(1,4)]
colnames(data_liger) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_liger, grepl('Embryonic-Stem-Cell|H1ESC', X))
data2 = dplyr::filter(data_liger, !grepl('Embryonic-Stem-Cell|H1ESC', X))
...
data_scepi <- read.csv('h1esc_scepi_tsne.txt', header = TRUE,sep=" ")
data1 = dplyr::filter(data_scepi, grepl('Embryonic-Stem-Cell|H1ESC', X))
data2 = dplyr::filter(data_scepi, !grepl('Embryonic-Stem-Cell|H1ESC', X))
