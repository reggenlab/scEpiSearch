...
data_seurat <- read.csv('embeds-MCA-cell-Mouse-endothelial.txt', header = TRUE,sep="\t")
head(data_seurat,3)
data_seurat = data_seurat[ , c(-1)]
colnames(data_seurat) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_seurat, grepl('Endo|Endothelial_Mouse', X))
data2 = dplyr::filter(data_seurat, !grepl('Endo|Endothelial_Mouse', X))
...
data_conos <- read.csv('endothelial_mouse_conos_tsne.txt.csv',header=TRUE,sep=",")
head(data_conos,3)
data_conos <- data_conos[-c(1,2,6,7,8,9,10,11,12)]
colnames(data_conos) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_conos, grepl('Endo|endothelial_mouse', X))
data2 = dplyr::filter(data_conos, !grepl('Endo|endothelial_mouse', X))
...
data_liger <- read.csv('liger_endothelial_mouse_tsne.txt', header = TRUE,sep="\t")
head(data_liger,3)
#data_liger <- data_liger[-c(1)]
colnames(data_liger) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_liger, grepl('Endo|Endothelial_mouse', X))
data2 = dplyr::filter(data_liger, !grepl('Endo|Endothelial_mouse', X))
...
data_scepi <- read.csv('endothelial_liver_scepi_tsne.txt', header = TRUE,sep=" ")
data1 = dplyr::filter(data_scepi, grepl('Endo|Endothelial_Mouse', X))
data2 = dplyr::filter(data_scepi, !grepl('Endo|Endothelial_Mouse', X))
...
sil_scepi = dplyr::filter(avg_sil_scepi, grepl('Endothelial_Mouse', X))

avg_sil_seurat = as.data.frame(avg_sil_seurat)
avg_sil_seurat$X = labels_seurat
sil_seurat = dplyr::filter(avg_sil_seurat, grepl('Endothelial_Mouse', X))

avg_sil_conos = as.data.frame(avg_sil_conos)
avg_sil_conos$X = labels_conos
sil_conos = dplyr::filter(avg_sil_conos, grepl('endothelial_mouse', X))

avg_sil_liger = as.data.frame(avg_sil_liger)
avg_sil_liger$X = labels_liger
sil_liger = dplyr::filter(avg_sil_liger, grepl('Endothelial_mouse', X))
