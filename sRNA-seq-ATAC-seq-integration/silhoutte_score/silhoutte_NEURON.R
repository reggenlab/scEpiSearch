...
data_seurat <- read.csv('embeds-MCA-cell-Human-neuron.txt', header = TRUE,sep="\t")
head(data_seurat,3)
data_seurat = data_seurat[ , c(-1)]
colnames(data_seurat) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_seurat, grepl('neuron|neuron_human', X))
data2 = dplyr::filter(data_seurat, !grepl('neuron|neuron_human', X))
...
data_conos <- read.csv('neuron_conos_tsne.txt',header=TRUE,sep=",")
head(data_conos,3)
data_conos <- data_conos[-c(1)]
colnames(data_conos) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_conos, grepl('neuron|neuron_human', X))
data2 = dplyr::filter(data_conos, !grepl('neuron|neuron_human', X))
...
data_liger <- read.csv('neuron_liger_tsne.csv', header = TRUE,sep=",")
head(data_liger,3)
#data_liger <- data_liger[-c(1,4)]
colnames(data_liger) = c("TSNE_1","TSNE_2","X")
data1 = dplyr::filter(data_liger, grepl('neuron|neuron_human', X))
data2 = dplyr::filter(data_liger, !grepl('neuron|neuron_human', X))
...
data_scepi <- read.csv('neuron_scepi_tsne.txt', header = TRUE,sep=" ")
data1 = dplyr::filter(data_scepi, grepl('neuron|neuron_human', X))
data2 = dplyr::filter(data_scepi, !grepl('neuron|neuron_human', X))
...
sil_scepi = dplyr::filter(avg_sil_scepi, grepl('neuron_human', X))

avg_sil_seurat = as.data.frame(avg_sil_seurat)
avg_sil_seurat$X = labels_seurat
sil_seurat = dplyr::filter(avg_sil_seurat, grepl('neuron_human', X))

avg_sil_conos = as.data.frame(avg_sil_conos)
avg_sil_conos$X = labels_conos
sil_conos = dplyr::filter(avg_sil_conos, grepl('neuron_human', X))

avg_sil_liger = as.data.frame(avg_sil_liger)
avg_sil_liger$X = labels_liger
sil_liger = dplyr::filter(avg_sil_liger, grepl('neuron_human', X))
